...
Scrolling is smoothed by Lenis, pinned to a single GSAP ticker that also drives both WebGL canvases, so scroll position and the rendered scene stay on the same frame. The document remains the native scroller: hash links, keyboard scrolling, browser scroll restoration and touch scrolling are unchanged, and the stylesheet keeps native smooth scrolling as the no-JavaScript fallback. Nested scrollable regions such as the mobile menu and the media dialog opt out.

The background is a faceted glass torus knot rendered behind content and capped at 60 FPS with drift-free scheduling. Its surface is displaced on the GPU and lit by a small environment generated in-scene, so it downloads nothing. Desktop uses 2,880 vertices with physical transmission at half resolution; mobile and lower-capability devices start at 1,050 vertices with transmission disabled and at most 180,000 drawing-buffer pixels. Sustained slow frames reduce quality to 297 vertices/90,000 pixels, and persistent pressure freezes the surface rather than fighting the browser. Quality never ramps back up during the same mount. Device pixel ratio is capped at 2 and further limited to 1 on desktop and 0.75 below it. Shadow maps stay disabled. Reduced motion, pause and hidden tabs stop the loop; the hero canvas also stops while scrolled away. Low-capability touch devices keep the portrait image instead of running a second WebGL canvas. Optional device tilt remains opt-in, and an opt-in hero control shapes the sculpture by drag or arrow keys without capturing page scrolling. Real-device GPU speed, battery state, and thermal limits vary, so flawless 60 FPS or no overheating cannot be guaranteed.

Scroll depth drives the scene: measured section markers map to camera, position, rotation, morph and transmission targets, so the background shifts as each section is read. Interior pages and reading layouts use calmer poses, and print output drops the canvas entirely.
...
Scene visual tests enable framebuffer retention only through test instrumentation for deterministic pixel capture; performance tests exercise the normal production setting with retention disabled. Because a glass frame issues several draw calls, the browser tests count rendered frames by clustering draw timestamps rather than counting draw calls. A separate mobile benchmark uses 4x Chromium CPU throttling, a DPR-3 viewport, and low-capability hints. Its short-run measurements are not a physical-phone GPU, battery, or thermal certification.

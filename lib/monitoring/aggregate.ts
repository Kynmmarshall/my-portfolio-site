...
export function summarizeSamples(samples: Sample[], now = Date.now()) {
  ...
  const successes = known.filter(
    (sample) => sample.result === "reachable",
  ).length;
  return {
    availability: known.length >= 12 ? (successes / known.length) * 100 : null,
    coverage: Math.min(100, (known.length / expected) * 100),
    observed: known.length,
    expected,
    since: first.checkedAt,
  };
}
